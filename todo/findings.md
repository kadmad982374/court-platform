...
- [x] ~~**P8b-02** 🟠 **`GlobalExceptionHandler` returns 500 for framework exceptions**~~ — ✅ **Fixed in image `:03` (2026-04-24)**. Added `@ExceptionHandler(NoResourceFoundException.class)` → 404 `NOT_FOUND` and `@ExceptionHandler(HttpRequestMethodNotSupportedException.class)` → 405 `METHOD_NOT_ALLOWED` in `common/api/GlobalExceptionHandler.java`. Verified live.
- [x] ~~**P8b-03** 🟡 **Ex-lawyer loses case read access after reassignment**~~ — ✅ **Fixed in image `:03` (2026-04-24)** (partial — covers ReminderService path, which was the observed regression). Extended `CaseOwnershipPort` with `isAssignedLawyerOfAnyStage(caseId, userId)` (implemented in `CaseOwnershipAdapter`), added overload `AuthorizationService.requireReadAccessToCase(..., caseId)` that falls back to stage-assignment history for STATE_LAWYER, migrated `ReminderService.loadCaseAndRequireRead` to pass caseId. `AttachmentService` and `ExecutionService` still use the old signature — migrate those in a follow-up if ex-lawyer attachment access becomes an issue in practice.
- [x] ~~**P8b-04** 🟢 **Login action log shows "Anonymous — signed in"**~~ — ✅ **Fixed in image `:03` (2026-04-24)**. `AuthService.login()` now sets `MDC.put("username", user.getUsername())` (and `userId`) before calling `UserActionLog.action("signed in")`, in a try/finally that clears MDC. Log now shows `User 'section_fi_dam' signed in`.
